...
typedef enum {
  ...
  transcribe_file    = 0       // copy from infile to outfile
} encoding_from_to_t;

typedef struct
{
  format_t encoding_to;     // 4 bytes
  format_t encoding_from;   // 4 bytes
  int bom_length;           // 4 bytes
  char *in_file;            // 8 bytes
  char *out_file;           // 8 bytes + 4 bytes padding
} state_t;                  // 32 bytes
