...
/**
 * Test whether the -h flag is present anywhere in argv.
 * if it is present the program should return EXIT_SUCCESS and print USAGE.
 */
Test(my_args_suite, test_h_present) {
    char *jsa = NULL;
    int argc;

    // char *argv_0[] = {"./bin/utf", "-h"};
    // argc = 2;
    // info("testing with argc: %d, argv_0: %s", argc, (jsa = join_string_array(argc, argv_0)));
    // parse_args(argc, argv_0);
    // free(jsa);

    char *argv_1[] = {"./bin/utf", "-e", "-h"};
    argc = 3;
    info("testing with argc: %d, argv: %s", argc, (jsa = join_string_array(argc, argv_1)));
    parse_args(argc, argv_1);
    free(jsa);

    char *argv_2[] = {"./bin/utf", "-e", "UTF8", "-h"};
    argc = 4;
    info("testing with argc: %d, argv: %s", argc, (jsa = join_string_array(argc, argv_2)));
    parse_args(argc, argv_2);
    free(jsa);

    char *argv_3[] = {"./bin/utf", "-e", "UTF8", "./rsrc/sample_utf8bom.txt", "-h"};
    argc = 5;
    info("testing with argc: %d, argv: %s", argc, (jsa = join_string_array(argc, argv_3)));
    parse_args(argc, argv_3);
    free(jsa);

    char *argv_4[] = {"./bin/utf", "-e", "UTF8", "./rsrc/sample_utf8bom.txt", "./outfile.txt", "-h"};
    argc = 6;
    info("testing with argc: %d, argv: %s", argc, (jsa = join_string_array(argc, argv_4)));
    parse_args(argc, argv_4);
    free(jsa);
...
/**
 * If an output file is missing then the program will create one and continue.
 */
Test(my_args_suite, test_outfile_missing) {
    // int argc;

    // char *argv_0[] = {"./bin/utf", "-e", "UTF8", "./rsrc/sample_utf8bom", /* outfile missing!*/};
    // argc = 4;


}

Test(my_args_suite, test_inoutfile_missing){

}

Test(my_args_suite, test_same_inoutfile){
